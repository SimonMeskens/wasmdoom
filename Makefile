...
CC=emcc  # gcc or g++
CFLAGS+=$(SDL_CFLAGS) -sUSE_SDL=2 -gsource-map
LDFLAGS+=--preload-file doom1.wad --source-map-base http://localhost:3000/src/
LIBS+=-lm -lc $(SDL_LIBS)
...
$(OUTPUT):	$(OBJS)
	mkdir -p $(OUTDIR)
	@echo [Linking $@]
	$(VB)$(CC) $(CFLAGS) $(LDFLAGS) $(OBJS) -o $(OUTDIR)/$(OUTPUT).js $(LIBS)
	cp index.html $(OUTDIR)/index.html
	cp doom1.wad $(OUTDIR)/doom1.wad
